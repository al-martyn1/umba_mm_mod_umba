...
struct IPollCapable : UMBA_INHERITS umba::IUnknown
{
    ...
    //! Метод для полировки. Возвращает true, если были произведены какие-то действия. Зависит от конкретной реализации, и является простым способом что-то сообщить наверх. После отладки результат тупо игнорируется.
    virtual
    bool poll() = 0;
